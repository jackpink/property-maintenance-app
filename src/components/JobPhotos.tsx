...
const Photos: React.FC<Props> = ({ photos, rooms }) => {
    ...
    return(
        <div >
            <Popover popoveropen={roomAssignButtonsOpen} setPopoverOpen={setRoomAssignButtonsOpen}>
                <div className="py-28 text-center justify-center flex flex-col">
                    <h1 className="text-lg">Assign Selected Photos to Room</h1>
                    {rooms.map((room, index) => {
                        return(
                            <Button onClick={click} value={room.id} className="my-4 md:w-96" key={index} >{room.Level.label.toUpperCase() + "→" +room.label.toUpperCase()}</Button>
                        )
                    })}
                </div>
            </Popover>
            <div className="flex flex-wrap gap-4 pt-8 justify-center">
                ...
                {(assignMode) ? (
                    <div className="fixed bottom-6 w-full">
                        <Button onClick={() => setRoomAssignButtonsOpen(true)} className="p-6 w-2/3">Move to Room</Button>
                        <Button onClick={exitAssignMode} className="p-6 ml-5">X</Button>
                    </div>
                    ) : (
                        <></>
                    )}
